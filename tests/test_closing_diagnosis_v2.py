...
def test_diagnostico_v2_limita_lacunas_e_fica_recolhido():
    script = Path('static/closing_diagnosis_v2.js').read_text(encoding='utf-8')
    assert 'qtd > 20' in script
    assert 'modelo+série+mês' in script or 'modelo, série e mês' in script
    assert "id=\"omnixml-diag-conteudo\" class=\"hidden" in script
    assert 'Lacuna plausível' in script
